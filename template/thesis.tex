...
\begin{frontmatter}
  \begin{abstract}
      ...
      Mit diesem Ansatz können so ML-Algorithmen und die Konfigurationsoptionen mit einer sehr hohen Genauigkeit identifiziert
      werden.
...
\tikzstyle{form} = [rectangle, rounded corners, minimum height=1.33cm,text centered, text width=4cm, draw=black]
\tikzstyle{arrow} = [thick,->,>=stealth, text width=2.8cm, text centered]

\begin{center}
\begin{tikzpicture}[node distance=2cm]
\node (start) [form] {Python-Code als AST parsen};
\coordinate[left of=start, xshift=-0.8cm] (d2);
\draw [arrow] (d2) -- node[anchor=east] {JSON-Datei} (start);
\node (first) [form, above of=start, yshift=-4cm] {AST-Knoten nach ML-Imports durchsuchen};
\draw [arrow] (start) -- (first);
\node (second) [form, above of=first, yshift=-4.23cm] {AST-Knoten filtern, die ML-Imports enthalten};
\draw [arrow] (first) -- (second);
\node (third) [form, above of=second, yshift=-4.23cm] {AST-Knoten, die ML-Klassen enthalten, als Dictionary speichern};
\draw [arrow] (second) -- (third);
\coordinate[right of=third, xshift=0.8cm] (d1);
\draw [arrow] (third) -- node[anchor=west] {Liste mit Dictionaries} (d1);
\end{tikzpicture}
\end{center}
\
...
\begin{center}
\setlength{\tabcolsep}{5pt}
\begin{tabular}[h]{l|c|c|c|c|c|c} %p{7cm}
\hline
