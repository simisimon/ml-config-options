...
class SSA:
    ...
    def get_stmt_idents_ctx(self, stmt, del_set=[], const_dict = {}):
        ...
        if isinstance(stmt, ast.Assign):
            ...
            if len(targets) == 1:
                if hasattr(targets[0], "id"):
                    ...
                    left_name = ast.unparse(stmt.targets[0]) ############ Marco
                    const_dict[left_name] = stmt.value  ############## Marco
        ...
        for r in ident_info:
            if r['name'] is None or "_hidden_" in r['name']:    ######### Marco
                continue
            if r['usage'] == 'store':
                stored_idents.append(r['name'])
            if r['usage'] == 'load': #################### Marco
                stored_idents.append(r['name']) ################## Marco
            else:
                loaded_idents.append(r['name'])
            if r['usage'] == 'del':
