...
class KMedians(BaseEstimator, ClusterMixin):
    ...
    def __init__(self, n_clusters=4, metric='manhattan', method='per-axis', init='random', max_iter=300, tol=0.0001,
                 random_state=None):
    ...
    def _check_arguments(self):
        ...
        if self._is_nonnegative(self.n_clusters, "n_clusters") and self._is_nonnegative(self.max_iter,
                                                                                        "max_iter") and isinstance(
                self.tol, (float, np.floating)):
            pass
        else:
            raise ValueError("Tolerance is not specified as floating-point value")

        if (isinstance(self.random_state,
                       (int, np.integer, None, np.random.RandomState)) or self.random_state is np.random):
            pass
        else:
    ...
    def fit(self, X, Y=None):
        ...
        if self.n_clusters > n_samples:
            raise ValueError(
                'Number of clusters %s cannot be greater than number of samples %s' % (self.n_clusters, n_samples))
